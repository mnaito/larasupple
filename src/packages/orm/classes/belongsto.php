...
class BelongsTo extends Relation
{
	...
	public function __construct($from, $name, array $config)
	{
		...
		{
			throw new \Mits430\Larasupple\Packages\FuelException('Related model not found by Belongs_To relation "'.$this->name.'": '.$this->model_to);
		}
		...
		elseif ($current_model_id != null)
		{
			...
			if ($changed)
			{
				...
				if ( ! is_null($new_rel_id))
				{
					...
					if (empty($rel_obj))
					{
						throw new \Mits430\Larasupple\Packages\FuelException('New relation set on '.$this->model_from.' object wasn\'t found.');
					}
				}
				else
				{
